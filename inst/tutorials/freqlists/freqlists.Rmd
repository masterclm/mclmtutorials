...
In this documentation^[You can also go to [the online documentation](https://masterclm.github.io/mclm/reference/freqlist.html).], we see that the default value for the argument `re_token_splitter` is `re("[^_\\p{L}\\p{N}\\p{M}'-]+")`.^[In "strings literals" in R, any backslash needs to be doubled. See the regular expressions tutorial (`learnr::run_tutorial("regex", package = "mclm")`) for more information.]
...
For instance, for *the/at*, the normalized frequency is calculated^[In code: `flist1[["the/at"]]] / tot_n_tokens(flist1) * 10000`.] as:
...
You can also *view* the frequency list by typing, on the console, `View(flist)`, `flist` being the name of your frequency list, i.e. the variable in which it was stored. In addition, you can save your frequency list to file with `write_freqlist(flist, "my_first_freqlist.tab")` (with whatever path you want to store it to) and then read it back, in a different R session for example, with `flist <- read_freqlist("my_first_freqlist.tab")`.

It is also possible to open the frequency list with e.g. LibreOffice Calc (or Excel), since the frequency list is just a simple 'tab separated values' file. In LibreOffice Calc, just choose `File | Open...`, select the file
...
Therefore, for this corpus and this value of `re_token_transf_in`, any mention of `\1` in `token_transf_out` will reproduce the contents of the first group, i.e. the word form, and any mention of `\2` will reproduce the contents of the second group, i.e. the POS tag.
...
First we use the `re_drop_token` argument. We run the following code to obtain the keyness measure for *the/at* according to this first approach.

```{r key1, exercise = TRUE, exercise.eval = TRUE}
...
```{r key2, exercise = TRUE, exercise.eval = TRUE}
flist_ca01 <- freqlist("data/ca01", 
  re_token_splitter = r"--[(?xi)   \s+          ]--") %>%
  keep_re(r"--[(?xi)   / [\p{L}]   ]--")
